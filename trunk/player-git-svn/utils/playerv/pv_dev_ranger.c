...
intg=g*255.0;
intb=b*255.0;
if (intr>255){intr=255;}if (intr<0){intr=0;}
if (intg>255){intg=255;}if (intg<0){intg=0;}
if (intb>255){intb=255;}if (intb<0){intb=0;}
rgb=(intr<<16)+(intg<<8)+intb;
return rgb;
...
int color=0;
  ...
  if (ranger->proxy->element_count > 1)
  {
	...
	color=	rgbHue(((float)(ranger->proxy->intensities[ii]))/255.*0.6);
      ...
      rtk_fig_color_rgb32(ranger->scan_fig[ii], color);
  ...
  else
  {
    ...
    if (rtk_menuitem_ischecked(ranger->style_item))
    {
      ...
      rtk_fig_color_rgb32(ranger->scan_fig[0], color);
