...
extern int showFootprints_Flag;
extern int showFootprintsMap_Flag;
extern int showRobotName_Flag;
...
_show_names(GtkWidget *widget,
		...
		gpointer data)
{
	showRobotName_Flag = !showRobotName_Flag;
	return(TRUE);
}
...
_show_footprints(GtkWidget *widget,
		...
		gpointer data)
{
	showFootprints_Flag = !showFootprints_Flag;
		...
		gpointer data)
{
	...
	if(gui_data->mapVW_idx != tmpVW_idx)  // 更改了 mapIndex
	{
		...
		showVirtualWall_Flag = subscribe_and_read_map(gui_data, gui_data->mapVW, gui_data->mapVW_idx, virtualwall_item, 0);
	}
	else
		...
		gpointer data)
{
	...
	if(gui_data->mapZONE_idx != tmpZONE_idx )  // 更改了 mapIndex
	{
		...
		showZoneMap_Flag = subscribe_and_read_map(gui_data, gui_data->mapZONE, gui_data->mapZONE_idx, zone_item, 1);
	}
	else
		...
		gpointer data)
{
	...
	if(gui_data->mapPOWER_idx != tmpPOWER_idx)  // 更改了 mapIndex
	{
		...
		showPowerMap_Flag = subscribe_and_read_map(gui_data, gui_data->mapPOWER, gui_data->mapPOWER_idx, power_item, 2);
	}
	else
		...
		gpointer data)
{
	...
	switch(event->type)
	{
		case GDK_BUTTON_PRESS:  // 按下按鈕
			...
			{
				case 2: // Middle button enables/disables the robot
					if(onrobot && !setting_theta)
					{
						gui_data->robot_enable_state = !gui_data->robot_enable_state;
						playerc_planner_enable(gui_data->planner, gui_data->robot_enable_state, 0);
					}
					break;
				case 3: // Right button
...
void make_menu(gui_data_t* gui_data)
{
	...
	GtkCheckMenuItem* footprints_item; // playernav2 self record
	...
	show_names_item     = (GtkCheckMenuItem*)gtk_check_menu_item_new_with_label("Show robot name");
	footprints_item     = (GtkCheckMenuItem*)gtk_check_menu_item_new_with_label("Show robot footprints");
	footprints_item_map = (GtkCheckMenuItem*)gtk_check_menu_item_new_with_label("Show robot footprints by Map70");
	...
	make_map_menu(gui_data, VW_START, VW_END, vwIdx_menu, virtualwall_mapIdx_item, 0);
	make_map_menu(gui_data, ZONE_START, ZONE_END, zoneIdx_menu, zone_mapIdx_item, 1);
	make_map_menu(gui_data, POWER_START, POWER_END, powerIdx_menu, power_mapIdx_item, 2);
...
ex:  Ctrl-s screen shot     */
	...
	/*  gtk_widget_add_accelerator((GtkWidget*)virtualwall_item, "activate",
	...
	g_signal_connect_swapped(G_OBJECT (footprints_item_map), "activate",
			...
			(gpointer) gui_data);
	/*  g_signal_connect_swapped(G_OBJECT (virtualwall_item), "activate",
		G_CALLBACK(_show_virtualwall),
		(gpointer) gui_data);*/
...
void init_gui(gui_data_t* gui_data, int argc, char** argv)
{
	...
	// 重要！設定縮放大小，對執行時間影響很大。
	initial_zoom = (gui_data->initial_zoom) * 16 /*(1.0 / gui_data->map->resolution)*/;
	// zoom in at most 10 times
	max_zoom = 10.0 * initial_zoom;

	g_assert((gui_data->zoom_adjustment =
				(GtkAdjustment*)gtk_adjustment_new(initial_zoom,  // 滾動條當前的位置。這個變量必須處於lower和upper值之間
					initial_zoom,  // 滾動條upper值
					max_zoom,      // 滾動條lower值
					(max_zoom-initial_zoom)/1e3, // 設置步長 -> 點擊向下箭頭
					(max_zoom-initial_zoom)/1e2, // 設置步長 -> 按下向下翻頁鍵
					(max_zoom-initial_zoom)/1e2))); // page_size是在屏幕上一次可見的區域，它取決於構件的大小。
...
void create_map_image(gui_data_t* gui_data, int ori_X, int ori_Y, int draw_width, int draw_height)
{
	...
	for(j=ori_Y; j < draw_height; j++) {
		for(i=ori_X; i < draw_width; i++) {
			...
			if(showFootprintsMap_Flag && gui_data->mapFP->cells[PLAYERC_MAP_INDEX(gui_data->mapFP,i,j)] == 1)
			{
...
void create_robot(gui_data_t* gui_data, pose_t pose)
{
	...
	update_robot_name(gui_data, pose);
...
void move_item(GnomeCanvasItem* item, pose_t pose, int raise, int showMes)
{
	...
	if(showMes)
	{
		...
		platPos_to_mapPos(&goal_data, pose);
...
{
	// 化為地圖單位
	gui_data->onmap_pose.px = (robot_pose.px - mapOrigin[0]) / mapResolution + 0.5;
	gui_data->onmap_pose.py = (robot_pose.py - mapOrigin[1]) / mapResolution + 0.5;
}
...
void draw_waypoints(gui_data_t* gui_data)
{
	...
	else if(gui_data->planner->path_valid)
	{
		...
		for( i = MAX(gui_data->planner->curr_waypoint-1, 0);
				...
				i++)
		{
			if(i < gui_data->planner->waypoint_count-1)
			{
				g_assert((waypoint =
