...
void playerc_audio_destroy(playerc_audio_t *device)
{
  ...
  free(device->mixer_data.channels);
  free(device);
}
//-------------------------------------------------------------------------------------------------
                             ...
                             uint8_t *data, size_t len)
{
  ...
  else if((header->type == PLAYER_MSGTYPE_DATA) && (header->subtype == PLAYER_AUDIO_DATA_MIXER_CHANNEL))
  {
    ...
    if ( device->mixer_data.channels )
      free(device->mixer_data.channels);

    device->mixer_data.channels=malloc(sizeof(player_audio_mixer_channel_t)*device->mixer_data.channels_count);
...
//-------------------------------------------------------------------------------------------------
int playerc_audio_stop_play_cmd(playerc_audio_t *device)
{
  return playerc_client_write(device->info.client, &device->info,
                              PLAYER_AUDIO_CMD_STOP_PLAY,
                              NULL, NULL);
}
//-------------------------------------------------------------------------------------------------
