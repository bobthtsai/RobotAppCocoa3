...
int timed_recv(int s, void *buf, size_t len, int flags, int timeout)
{
  ...
  if (ret <= 0)
  {
    ...
    {
      PLAYERC_ERR1("poll call timed out with no data to receive timeout=%d", timeout);
      return ret;
    }
    else
    {
...
// Read and process a packet (blocking)
void *playerc_client_read_timeout(playerc_client_t *client, int timeout)
{
  void* ret_proxy = NULL;
  int ret;
