...
playerc_laser_t *playerc_laser_create(playerc_client_t *client, int index)
{
  ...
  device->calibration.type=PLAYER_LASER_CALIBRATION_NONE;
                          ...
                          void *data)
{
          ...
          (header->subtype == PLAYER_LASER_DATA_SCANANGLE))
  {
	  ...
	  device->scan_id = scan_data->id;
          ...
          (header->subtype == PLAYER_LASER_DATA_CALIBRATION))
  {
    device->calibration = *(player_laser_calibration_t*)data;
  }
  else
...
//-------------------------------------------------------------------------------------------------
int playerc_laser_enable( playerc_laser_t * device,
    BOOL enable )
{
...
//-------------------------------------------------------------------------------------------------
int
playerc_laser_set_calibration(playerc_laser_t *device,
    uint8_t type,
    ...
    uint16_t index_end)
{
  ...
  device->calibration.type=calibration.type = type;
  calibration.index_start = index_start;
  calibration.index_end = index_end;


  return playerc_client_write(device->info.client, &device->info,
                   PLAYER_LASER_CMD_CALIBRATION,
                   &calibration, NULL);
}
//-------------------------------------------------------------------------------------------------
int
playerc_laser_get_calibration(playerc_laser_t *device,
    uint8_t *type,
    float *angle,
    float *distance)
{
  *type = device->calibration.type;
  *angle = device->calibration.correct_angle;
  *distance = device->calibration.correct_distance;
