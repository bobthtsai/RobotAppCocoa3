...
#if defined (WIN32)
#define PLAYER_STATIC //BOBMODIFY
...
/** @brief Command to stop audio play immediately */
PLAYERC_EXPORT int playerc_audio_stop_play_cmd(playerc_audio_t *device);
...
typedef struct
{
  ...
  player_dio_data_array_t data_array;
} playerc_dio_t;
...
PLAYERC_EXPORT int playerc_dio_set_output_array(playerc_dio_t *device, uint8_t num, player_dio_cmd_t *dio);
...
typedef struct
{
  ...
  player_laser_calibration_t calibration;
} playerc_laser_t;
...
/** @brief control laser power default is power on. */
PLAYERC_EXPORT int playerc_laser_enable( playerc_laser_t * device,
    BOOL power_en );


PLAYERC_EXPORT int playerc_laser_set_calibration(playerc_laser_t *device,
    uint8_t type,
    uint16_t index_start,
    uint16_t index_end);

PLAYERC_EXPORT int playerc_laser_get_calibration(playerc_laser_t *device,
    uint8_t *type,
    float *angle,
    float *distance);
...
/** @brief Get the map info, which is stored in the proxy. */
PLAYERC_EXPORT int playerc_map_get_map_info(playerc_map_t* device);

/** @brief Get the map, which is stored in the proxy. */
PLAYERC_EXPORT int playerc_map_get_partial_map(playerc_map_t* device,uint32_t col,uint32_t row,uint32_t width,uint32_t height, BOOL compress);

/** @brief Get the map, which is stored in the proxy. */
PLAYERC_EXPORT int playerc_map_get_map(playerc_map_t* device);

/** @brief Get the map for python, which is stored in the proxy. */
PLAYERC_EXPORT int playerc_map_get_mapp(playerc_map_t* device);

/** @brief Get the vector map, which is stored in the proxy. */
PLAYERC_EXPORT int playerc_map_get_vector(playerc_map_t* device);

/** @brief Set the whole map data to plugin. */
PLAYERC_EXPORT int playerc_map_set_map(playerc_map_t *device,char *map_data);

/** @brief Set the map data to plugin. (Using zlib to compress data.) */
PLAYERC_EXPORT int playerc_map_set_partial_map(playerc_map_t *device,char *map_data,uint32_t col,uint32_t row,uint32_t width,uint32_t height);

/** @brief Set the map cmd to plugin ask map to save as a file.*/
PLAYERC_EXPORT int playerc_map_save(playerc_map_t *device);

/** @brief Set the map cmd to plugin ask map to load frome file.*/
PLAYERC_EXPORT int playerc_map_load(playerc_map_t *device);

/** @brief Set the map info, which is stored in the proxy.The important note is that the map data will be cleaned
           if you set  width or height zero. You can also use this interface to lock/unlock map. */
PLAYERC_EXPORT int playerc_map_set_map_info(playerc_map_t* device, player_map_info_t *info_req);
...
/** @brief Set a list of goals' pose (gx, gy, ga) */
PLAYERC_EXPORT int playerc_planner_set_cmd_pose_list(playerc_planner_t *device,
             player_pose2d_t *list, uint32_t count, uint32_t mapIndex);
...
*/
PLAYERC_EXPORT int playerc_planner_enable(playerc_planner_t *device, int state, int explore_state);
...
/** Note: the structure describing the Dispatcher node's data packet is declared in
          Player. */

/** @brief dispatcher proxy data. */
typedef struct
{
    /** Device info; must be at the start of all device structures.         */
  playerc_device_t info;
  ...
  uint8_t isSetPmCmd;
  player_dispatcher_ui_time_t uiTime;
  uint8_t isSetUiTime;
  uint8_t isReqUiTime;
  uint8_t isReqAckUiTime;
  player_dispatcher_ui_schedule_t uiSchedule;
  uint8_t isSetUiSchedule;
  uint8_t isReqUiSchedule;
  uint8_t isReqAckUiSchedule;
  uint8_t *fileData;
  player_dispatcher_status_t statusData;
  uint8_t isSetUiCmd;
  player_dispatcher_ui_cmd_data_t uiCmdData;
  player_dispatcher_ui_name_t uiName;
  uint8_t isSetUiName;
  uint8_t isReqUiName;
  uint8_t isReqAckUiName;
  player_dispatcher_ai_status_t aiStatus;
  player_dispatcher_ui_status_t uiStatus;
  player_dispatcher_pm_status_t pmStatus;
} playerc_dispatcher_t;

PLAYERC_EXPORT playerc_dispatcher_t *dispatcher_create (playerc_client_t *client, int index);

PLAYERC_EXPORT void dispatcher_destroy (playerc_dispatcher_t *device);

PLAYERC_EXPORT int dispatcher_subscribe (playerc_dispatcher_t *device, int access);

PLAYERC_EXPORT int dispatcher_unsubscribe (playerc_dispatcher_t *device);

/** Allow another client send command to power manager client via dispatcher. */
PLAYERC_EXPORT int dispatcher_set_cmd_pm(playerc_dispatcher_t *device, uint8_t initiator, uint32_t cmd);

/** Allow another client send command to AI client via dispatcher. */
PLAYERC_EXPORT int dispatcher_set_cmd_ai(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t cmd,
    uint32_t power,
...
/** AI client update current states to  dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_set_status_ai(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t aiStatus,
    uint32_t power,
    uint32_t error,
...
/** Power manager client update current states to  dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_set_status_pm(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t pmStatus,
    uint32_t error);

/** User interface client update current states to  dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_set_status_ui(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t uiStatus);

/** Allow another client to get the AI status via dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_get_status_ai(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t *aiStatus,
...
/** Allow another client to get the power manager status via dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_get_status_pm(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t *pmStatus,
    uint32_t *error);

/** Allow another client to get the user interface status via dispatcher-plugin. */
PLAYERC_EXPORT int dispatcher_get_status_ui(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t *uiStatus);

/** Set the time to user interface client. */
PLAYERC_EXPORT int dispatcher_set_ui_time(playerc_dispatcher_t *device, uint8_t initiator, player_dispatcher_time_t time, player_dispatcher_timezone_t zone);

/** Set the schedule to user interface client. */
PLAYERC_EXPORT int dispatcher_set_ui_schedule(playerc_dispatcher_t *device, uint8_t initiator, uint32_t count,  player_dispatcher_ui_alarm_time_t data[]);

/** User interface client get the time setting by another client. */
PLAYERC_EXPORT int dispatcher_get_ui_time(playerc_dispatcher_t *device, uint8_t *initiator, player_dispatcher_time_t *time, player_dispatcher_timezone_t *zone);

/** User interface client get the schedule setting by another client. */
PLAYERC_EXPORT player_dispatcher_ui_alarm_time_t * dispatcher_get_ui_schedule(playerc_dispatcher_t *device, uint8_t *initiator, uint32_t *count);

/** user interface client get the request information and handle it. */
PLAYERC_EXPORT int dispatcher_get_req_ui_time(playerc_dispatcher_t *device, uint8_t *initiator);

/** user interface client request ack information send back the initiator client. */
PLAYERC_EXPORT int dispatcher_set_req_ack_ui_time(playerc_dispatcher_t *device, uint8_t destination, player_dispatcher_time_t time, player_dispatcher_timezone_t zone);

/** user interface client get the request information and handle it. */
PLAYERC_EXPORT int dispatcher_get_req_ui_schedule(playerc_dispatcher_t *device, uint8_t *initiator);

/** user interface client request ack information send back the initiator client. */
PLAYERC_EXPORT int dispatcher_set_req_ack_ui_schedule(playerc_dispatcher_t *device, uint8_t destination, uint32_t count,  player_dispatcher_ui_alarm_time_t data[]);

/** remote device request to set file it is control by dispatcher. */
PLAYERC_EXPORT int dispatcher_set_file(playerc_dispatcher_t *device,
    uint8_t initiator,
    const int8_t *name,
    uint32_t name_size,
    const int8_t *data,
...
/** remote device request to get file it is control by dispatcher. */
PLAYERC_EXPORT int8_t * dispatcher_get_file(playerc_dispatcher_t *device,
    uint8_t initiator,
    const int8_t *name,
    uint32_t name_size,
    uint32_t *data_size);

/** client request to start pairing. */
PLAYERC_EXPORT int dispatcher_start_pairing(playerc_dispatcher_t *device,
		uint8_t initiator,
    const uint8_t *mac);

/** client<remode device> request login logout. */
PLAYERC_EXPORT int dispatcher_log_in_out(playerc_dispatcher_t *device,
    uint8_t log_in_out,
    const uint8_t *mac);

/** client<remode device> set command to UI. */
PLAYERC_EXPORT int dispatcher_set_cmd_ui(playerc_dispatcher_t *device,
    uint8_t initiator,
    uint32_t cmd );


/** Set the name to user interface client. */
PLAYERC_EXPORT int dispatcher_set_ui_name(playerc_dispatcher_t *device, uint8_t initiator, uint8_t *name);

/** User interface client get the name setting by another client. */
PLAYERC_EXPORT uint8_t *dispatcher_get_ui_name(playerc_dispatcher_t *device, uint8_t *initiator);

/** user interface client get the request information and handle it. */
PLAYERC_EXPORT int dispatcher_get_req_ui_name(playerc_dispatcher_t *device, uint8_t *initiator);

/** user interface client request ack information send back the initiator client. */
PLAYERC_EXPORT int dispatcher_set_req_ack_ui_name(playerc_dispatcher_t *device, uint8_t destination, uint8_t *name);
...
/** Note: the structure describing the keypad node's data packet is declared in
          Player. */

/** @brief keypad proxy data. */
typedef struct
{
    /** Device info; must be at the start of all device structures.         */
  playerc_device_t info;
  player_keypad_data_t keyPadData;
} playerc_keypad_t;

PLAYERC_EXPORT playerc_keypad_t *keypad_create (playerc_client_t *client, int index);

PLAYERC_EXPORT void keypad_destroy (playerc_keypad_t *device);

PLAYERC_EXPORT int keypad_subscribe (playerc_keypad_t *device, int access);

PLAYERC_EXPORT int keypad_unsubscribe (playerc_keypad_t *device);

PLAYERC_EXPORT int keypad_get_data(playerc_keypad_t *device, uint32_t *timestamp, uint8_t *key, uint8_t *status);

PLAYERC_EXPORT int keypad_get_config(playerc_keypad_t *device, uint32_t *de_bounce);

PLAYERC_EXPORT int keypad_set_config(playerc_keypad_t *device, uint32_t de_bounce);


/** Note: the structure describing the motor node's data packet is declared in
          Player. */

/** @brief motor proxy data. */
typedef struct
{
    /** Device info; must be at the start of all device structures.         */
  playerc_device_t info;
  player_motor_data_t motorData;
} playerc_motor_t;

PLAYERC_EXPORT playerc_motor_t *motor_create (playerc_client_t *client, int index);

PLAYERC_EXPORT void motor_destroy (playerc_motor_t *device);

PLAYERC_EXPORT int motor_subscribe (playerc_motor_t *device, int access);

PLAYERC_EXPORT int motor_unsubscribe (playerc_motor_t *device);

PLAYERC_EXPORT int motor_get_status (playerc_motor_t *device, uint8_t *state, float *current, float *watts, float *rpm, float *position);

PLAYERC_EXPORT int motor_set_pwm (playerc_motor_t *device, float pwm);

PLAYERC_EXPORT int motor_set_velocity (playerc_motor_t *device, float velocity, float acceleration);

PLAYERC_EXPORT int motor_set_position (playerc_motor_t *device, float velocity, float acceleration, float position);

PLAYERC_EXPORT int motor_set_delta_position (playerc_motor_t *device, float velocity, float acceleration, float deltaPosition);
...
/** Note: the structure describing the rtc node's data packet is declared in
          Player. */

/** @brief rtc proxy data. */
...
PLAYERC_EXPORT playerc_rtc_t *rtc_create (playerc_client_t *client, int index);

PLAYERC_EXPORT void rtc_destroy (playerc_rtc_t *device);

PLAYERC_EXPORT int rtc_subscribe (playerc_rtc_t *device, int access);

PLAYERC_EXPORT int rtc_unsubscribe (playerc_rtc_t *device);

PLAYERC_EXPORT int rtc_get_time (playerc_rtc_t *device, uint32_t *utc_time, int32_t *minuteswest, int32_t *dsttime);

PLAYERC_EXPORT int rtc_set_time (playerc_rtc_t *device, uint32_t utc_time, int32_t minuteswest, int32_t dsttime);

PLAYERC_EXPORT int rtc_get_alarm (playerc_rtc_t *device, uint32_t *time);

PLAYERC_EXPORT int rtc_set_alarm (playerc_rtc_t *device, uint32_t time);

PLAYERC_EXPORT int rtc_is_on_alarm (playerc_rtc_t *device, uint8_t req_type, uint8_t *is_on_alarm);
