...
//-------------------------------------------------------------------------------------------------
void
PlannerProxy::SetGoalPose(double aGx, double aGy, double aGa, uint32_t mapIndex)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_planner_set_cmd_pose(mDevice, aGx, aGy, aGa, mapIndex))
    throw PlayerError("PlannerProxy::SetGoalPose()", "error setting goal");
  return;
}
//-------------------------------------------------------------------------------------------------
void
PlannerProxy::SetGoalPose(player_pose2d_t *list, uint32_t count, uint32_t mapIndex)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_planner_set_cmd_pose_list(mDevice, list, count, mapIndex))
    throw PlayerError("PlannerProxy::SetGoalsPose()", "error setting goals");
  return;
}
//-------------------------------------------------------------------------------------------------
void
...
//-------------------------------------------------------------------------------------------------
void
PlannerProxy::SetEnable(bool aEnable, bool aExplore)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_planner_enable(mDevice, aEnable, aExplore))
