...
class PLAYERCC_EXPORT AudioProxy : public ClientProxy
{
  ...
  public:
    ...
    /** @brief Command to stop play audio */
    void PlayStop();
...
class PLAYERCC_EXPORT DioProxy : public ClientProxy
{
  ...
  public:
    ...
    uint32_t GetCount() const { return GetVar(mDevice->data.count); };

    /// A bitfield of the current digital inputs.
    uint32_t GetDigin() const { return GetVar(mDevice->data.bits); };
    ...
    /// Set the output to the bitfield aDigout
    void SetOutput(player_dio_cmd_t *dio, uint32_t num);
...
//      { return GetVar(mDevice->scan[aIndex]); };
    ...
    /// power enable/disable TRUE:enable FALSE:disable
    void Enable(bool enable);
    
    /// Allenc, 2014-11-25
    /// Extra API to get a copy of the ranges array.
    /// Allenc, 2014-12-08
    /// Use original Player designed API to get array copies.
    void GetRangeCopy(double *dst) {
      GetVarByRef(mDevice->ranges, mDevice->ranges + mDevice->scan_count, dst);
    };
    /// Allenc, 2014-11-25
    /// Extra API to get a copy of the intensity array.
    /// Allenc, 2014-12-08
    /// Use original Player designed API to get array copies.
    void GetIntensityCopy(int *dst) {
      GetVarByRef(mDevice->intensity, mDevice->intensity + mDevice->scan_count, dst);
    };

    void SetCalibration(uint8_t type,
        uint16_t index_start=0,
        uint16_t index_end=29);
    void GetCalibration(uint8_t *type,
        float *angle=NULL,
        float *distance=NULL);
};
...
class PLAYERCC_EXPORT MapProxy : public ClientProxy
{
  ...
  public:
    ...
    /// Refresh the partition of map in the proxy.(Use zlib to decompress data.)
    int RequestMap(uint32_t col,uint32_t row,uint32_t width,uint32_t height, bool compress=true);

    /// Set the map and store it in the proxy.
    int SetWholeMap(int8_t *map_data);

    /// Set the partition of map and store it in the proxy.(Using zlib to compress data.)
    int SetPartialMap(int8_t *map_data,uint32_t orgin_col,uint32_t orgin_row,uint32_t map_width,uint32_t map_height);

    /// Set Map info
    int SetMapInfo(float resolution,int width,int height,double orgin_x,double orgin_y);

    /// Get Map info
    void GetMapInfo(player_map_info_t *info); 
    ...
    /// Save plug-in map to file
    void Save();

    /// Load map from file
    void Load();

    /// Clean Plugin map data.
    void Clean();
    ...
    /// Transfer real world coordinate (meter) to map coordinate (pixel)
    ...
    /// Transfer map coordinate (pixel) to real world coordinate (meter)
...
class PLAYERCC_EXPORT PlannerProxy : public ClientProxy
{
  ...
  public:
    ...
    void SetGoalPose(double aGx, double aGy, double aGa, uint32_t mapIndex=0);

    /// Set the list of goals
    void SetGoalPose(player_pose2d_t *list, uint32_t count, uint32_t mapIndex=0);
    ...
    /// false to disable.
    void SetEnable(bool aEnable, bool aExplore = 0);
    ...
    int32_t GetIndexInGoalList(){
      scoped_lock_t lock(mPc->mMutex);
      return mDevice->goalidx;
...
class PLAYERCC_EXPORT Position2dProxy : public ClientProxy
{
  ...
  public:
    ...
    /// Is the device lost?
    float GetLost() const { return GetVar(mDevice->lost); };
};
...
/**
The @p DispatcherProxy class can be used to get images from a @ref
interface_dispatcher device. */
class PLAYERCC_EXPORT DispatcherProxy : public ClientProxy
{
  private:
    virtual void Subscribe(uint32_t aIndex);
    virtual void Unsubscribe();
    ...
    // libplayerc data structure
    playerc_dispatcher_t *mDevice;


  public:
    /// Constructor
    DispatcherProxy(PlayerClient *aPc, uint32_t aIndex=0);
    /// Destructor
    ~DispatcherProxy();

    /// get the data destination if the destination not match the client ID
    uint8_t DataDestination();

    /// start pairing
    int StartPairing( uint8_t initiator, const char *mac);

    /// client<remote device> login logout to dispatcher plug-in
    int LogIn(const char *mac);
    int LogOut(const char *mac);


    /** @brief Allow another client send command to AI client via dispatcher..
    @param initiator Where the command from
    @param cmd PLAYER_DISPATCH_AI_MOVE_FORWARD/PLAYER_DISPATCH_AI_AUTO etc....
    @param power PLAYER_DISPATCH_PM_AUTO/PLAYER_DISPATCH_PM_STANDBY etc...
    @param map_index_count How many map used
    @param map_index The map index number body<contain of *.cfg file map:0 map:1 map:(map_index)>
    @param target_point User want robot move to specific target point
    @param data_count Other data type data number the data unit: uint8_t
    @param data The data body
    @param stop_cond How to judge stop of one job*/
    void SetAiCmd(uint8_t initiator,
        uint32_t cmd=PLAYER_DISPATCH_AI_STOP,
        uint32_t power=PLAYER_DISPATCH_PM_STANDBY,
        uint32_t map_index_count=0,
        uint32_t *map_index=NULL,
        player_pose2d_t *target_point=NULL,
        uint32_t name_count=0,
        int8_t *name=NULL,
        uint32_t data_count=0,
        int8_t *data=NULL,
        uint32_t stop_cond=PLAYER_DISPATCH_AI_COND_JOB_DONE);
    /// allow AI client to query is another client to send command to AI
    bool IsSetAiCmd();
    /** @brief New command from another client let AI client to get the message..
    @param initiator Where the command from
    @param cmd PLAYER_DISPATCH_AI_MOVE_FORWARD/PLAYER_DISPATCH_AI_AUTO etc....
    @param power PLAYER_DISPATCH_PM_AUTO/PLAYER_DISPATCH_PM_STANDBY etc...
    @param targetPoint The body of target point*/
    void GetAiCmd(uint8_t *initiator, uint32_t *cmd, uint32_t *power, player_pose2d_t *targetPoint, uint32_t *stop_cond);
    uint32_t * GetAiMapLayer(uint32_t *map_index_count);
    int8_t * GetAiData(uint32_t *data_count);
    /// AI client update current states to  dispatcher-plugin.
    void SetAiStatus(uint8_t initiator,
        uint32_t aiStatus,
        uint32_t power,
        uint32_t error=0xFFFFFFFF,
        uint32_t worry=0xFFFFFFFF);
    /// Allow another client to get the AI status via dispatcher-plugin.
    void GetAiStatus(uint8_t initiator,
        uint32_t *aiStatus,
    ...
    /** @brief Allow another client send command to power manager client via dispatcher.
    @param initiator Where the command from
    @param cmd PLAYER_DISPATCH_PM_AUTO/PLAYER_DISPATCH_PM_STANDBY etc...*/
    void SetPmCmd(uint8_t initiator, uint32_t cmd);
    /// allow PM client to query is another client to send command to PM
    bool IsSetPmCmd();
    /// New command from another client let power manager client to get the message.
    void GetPmCmd(uint8_t *initiator, uint32_t *cmd);
    /// Power manager client update current states to  dispatcher-plugin.
    void SetPmStatus(uint8_t initiator, uint32_t pmStatus, uint32_t error=0xFFFFFFFF);
    /// Allow another client to get the power manager status via dispatcher-plugin.
    void GetPmStatus(uint8_t initiator, uint32_t *pmStatus, uint32_t *error);


    /// User interface client update current states to  dispatcher-plugin.
    void SetUiStatus(uint8_t initiator, uint32_t uiStatus);
    /// Allow another client to get the user interface status via dispatcher-plugin.
    void GetUiStatus(uint8_t initiator, uint32_t *uiStatus);
    /// allow user interface client to query is the time have been setting by another client
    bool IsSetUiTime();
    /// allow user interface client to query is the schedule have been setting by another client
    bool IsSetUiSchedule();
    /// allow user interface client to query is UI command have been setting by another client
    bool IsSetUiCmd();
    /// allow user interface client to query is another client to request the current time
    bool IsReqUiTime();
    /// allow user interface client to query is another client to request the current schedule
    bool IsReqUiSchedule();
    /// Set the time to user interface client.
    void SetUiTime(uint8_t initiator, player_dispatcher_time_t time, player_dispatcher_timezone_t zone);
    /// Set the schedule to user interface client.
    void SetUiSchedule(uint8_t initiator, uint32_t count, player_dispatcher_ui_alarm_time_t *pSchedule);
    /// User interface client get the time setting by another client.
    void GetUiTime(uint8_t *initiator, player_dispatcher_time_t *time, player_dispatcher_timezone_t *zone);
    /// User interface client get the schedule setting by another client.
    player_dispatcher_ui_alarm_time_t * GetUiSchedule(uint8_t *initiator, uint32_t *count);
    /// allow another client request user interface current time by blocking .
    void ReqUiTime(uint8_t initiator, player_dispatcher_time_t *time, player_dispatcher_timezone_t *zone );
    /// user interface client get the request information and handle it.
    void GetReqUiTime(uint8_t *initiator);
    /// user interface client request ack information send back the initiator client.
    void SetReqAckUiTime( uint8_t destination, player_dispatcher_time_t time, player_dispatcher_timezone_t zone);
    /// allow another client request user interface current schedule by blocking .
    player_dispatcher_ui_alarm_time_t *  ReqUiSchedule(uint8_t initiator, uint32_t *count);
    /// user interface client get the request information and handle it.
    void GetReqUiSchedule(uint8_t *initiator);
    /// user interface client request ack information send back the initiator client.
    void SetReqAckUiSchedule(uint8_t destination, uint32_t count,  player_dispatcher_ui_alarm_time_t data[]);
    /// set the content to the file
    void SetFile( uint8_t initiator,
        const char *name,
        uint32_t name_size,
        const char *data,
        uint32_t data_size);
    /// get the content to the file
    char * GetFile( uint8_t initiator,
        const char *name,
        uint32_t name_size,
        uint32_t *data_size);
    /// get the dispatcher status login=1 robot had been login login=0 robot did't login ;  pairing<0 is playerc_client_request error pairing=0 is playerc_client_request OK ; pairing>0 is playerc_client_request success but pairing error
    void Status( int8_t *login, int8_t *pairing);


    /** @brief Allow another client send command to UI client via dispatcher.
    @param initiator Where the command from
    @param cmd PLAYER_DISPATCH_UI_SWITCH_AP_MODE/PLAYER_DISPATCH_UI_SWITCH_CLIENT_MODE etc...*/
    void SetUiCmd(uint8_t initiator, uint32_t cmd);

    /// New command from another client let UI client to get the message.
    void GetUiCmd(uint8_t *initiator, uint32_t *cmd);

    /// allow user interface client to query is another client to request the current name
    bool IsReqUiName();
    /// allow user interface client to query is the name have been setting by another client
    bool IsSetUiName();
    /// allow user interface client to query is the name have been setting by another client
    void SetUiName(uint8_t initiator, uint8_t *name);
    /// User interface client get the name setting by another client.
    uint8_t *GetUiName(uint8_t *initiator);
    /// allow another client request user interface current name by blocking .
    uint8_t *ReqUiName(uint8_t initiator);
    /// user interface client get the request information and handle it.
    void GetReqUiName(uint8_t *initiator);
    /// user interface client request ack information send back the initiator client.
    void SetReqAckUiName( uint8_t destination, uint8_t *name );
};


/**
The @p KeypadProxy class can be used to get images from a @ref
interface_keypad device. */
class PLAYERCC_EXPORT KeypadProxy : public ClientProxy
{
private:
  virtual void Subscribe(uint32_t aIndex);
  virtual void Unsubscribe();
  ...
  // libplayerc data structure
  playerc_keypad_t *mDevice;

public:
  /// Constructor
  KeypadProxy(PlayerClient *aPc, uint32_t aIndex=0);
  /// Destructor
  ~KeypadProxy();

  void GetData(uint32_t *timestamp, uint8_t *key, uint8_t *status);
  void GetConfig(uint32_t *de_bounce);
  void SetConfig(uint32_t de_bounce);
};


/**
The @p MotorProxy class can be used to get images from a @ref
interface_motor device. */
class PLAYERCC_EXPORT MotorProxy : public ClientProxy
{
private:
  virtual void Subscribe(uint32_t aIndex);
  virtual void Unsubscribe();
...
public:
  /// Constructor
  MotorProxy(PlayerClient *aPc, uint32_t aIndex=0);
  /// Destructor
  ~MotorProxy();

  void GetStatus (uint8_t *state, float *current, float *watts, float *rpm, float *position);
  void SetPwm (float pwm);
  void SetVelocity (float velocity, float acceleration);
  void  SetPosition (float velocity, float acceleration, float position);
  void SetDeltaPosition (float velocity, float acceleration, float deltaPosition);
};


/**
The @p RtcProxy class can be used to get images from a @ref
interface_rtc device. */
class PLAYERCC_EXPORT RtcProxy : public ClientProxy
{
private:
  virtual void Subscribe(uint32_t aIndex);
  virtual void Unsubscribe();
  ...
  // libplayerc data structure
  playerc_rtc_t *mDevice;

public:
  /// Constructor
  RtcProxy(PlayerClient *aPc, uint32_t aIndex=0);
  /// Destructor
  ~RtcProxy();

  void GetTime(uint32_t *utc_time, int32_t *minuteswest=NULL, int32_t *dsttime=NULL);
  void SetTime( uint32_t utc_time, int32_t minuteswest=0, int32_t dsttime=0);
  void GetAlarm(uint32_t *time);
  void SetAlarm( uint32_t time);
  bool IsOnAlarm(uint8_t req_type=PLAYER_RTC_CLEAN_ALARM);
};
...
namespace std
{
  ...
  PLAYERCC_EXPORT std::ostream& operator << (std::ostream& os, const PlayerCc::DispatcherProxy& c);
  PLAYERCC_EXPORT std::ostream& operator << (std::ostream& os, const PlayerCc::KeypadProxy& c);
  PLAYERCC_EXPORT std::ostream& operator << (std::ostream& os, const PlayerCc::MotorProxy& c);
  PLAYERCC_EXPORT std::ostream& operator << (std::ostream& os, const PlayerCc::RtcProxy& c);
}
