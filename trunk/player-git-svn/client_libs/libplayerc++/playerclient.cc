...
void PlayerClient::Read()
{
  ...
  // first read the data
  {
    ...
    if(playerc_client_peek(mClient, 0) < 0)
      return ;
  }
  ...
  for (;;)
  {
...

    {
      ...
      ret = playerc_client_read_nonblock_withproxy(mClient, &ret_proxy);
    }
    if (ret < 0 || mClient->sock < 0)
      break;
    if(ret > 0)
      break;

    playerc_client_internal_peek(mClient, 50);
  }
