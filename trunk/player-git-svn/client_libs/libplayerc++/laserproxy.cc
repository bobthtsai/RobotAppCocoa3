...
//-------------------------------------------------------------------------------------------------
void
LaserProxy::Enable(bool enable)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_laser_enable(mDevice, (BOOL)enable) )
    throw PlayerError("LaserProxy::PowerEnable()", "error set power");

  return;
}
//-------------------------------------------------------------------------------------------------
void
LaserProxy::SetCalibration(uint8_t type,
uint16_t index_start,
uint16_t index_end)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_laser_set_calibration(mDevice, type, index_start, index_end) )
    throw PlayerError("LaserProxy::SetCalibration()");

  return;
}
//-------------------------------------------------------------------------------------------------
void
LaserProxy::GetCalibration(uint8_t *type,
    float *angle,
    float *distance)
{
  scoped_lock_t lock(mPc->mMutex);
  if (0 != playerc_laser_get_calibration(mDevice, type, angle, distance) )
    throw PlayerError("LaserProxy::GetCalibration()");
