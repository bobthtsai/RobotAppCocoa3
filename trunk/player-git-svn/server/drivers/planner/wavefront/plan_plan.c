...
_plan_update_plan(plan_t *plan, double lx, double ly, double gx, double gy)
{
  ...
  {
	printf("WAVEFRONT %s(): goal out of bounds.\r\n", __FUNCTION__);
  ...
  {
	  printf("WAVEFRONT %s(): start out of bounds.\r\n", __FUNCTION__);
  ...
  //printf("Cell + %d, I am in %f, %f. Occ = %d, Occ_dyn = %d\r\n", 
	//PLAN_INDEX(plan, li, lj),
	//lx, 
	//ly, 
	//cell->occ_state, 
	//cell->occ_state_dyn);
  ...
  if(!cell->plan_next)
  {
    ...
    printf("WAVEFRONT %s(): never found start in %u, %u, occ = %d, occ_dyn = %d\r\n", 
		__FUNCTION__, 
		cell->ci, 
		cell->cj, 
		cell->occ_state,
