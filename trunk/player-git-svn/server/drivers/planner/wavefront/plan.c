...
plan_compute_cspace(plan_t* plan)
{
  int i, j;
  int di, dj;
  int  half_kernel_width = plan->dist_kernel_width/2;
  float* p;
  plan_cell_t *cell, *ncell;
clock_t last_clock = clock();
  ...
  for (j = plan->min_y; j <= plan->max_y; j++)
  {
    ...
    for (i = plan->min_x; i <= plan->max_x; i++, cell++)
    {
      ...
      p = plan->dist_kernel;
      for (dj = -half_kernel_width; 
           dj <= half_kernel_width; 
           dj++)
      {
        ncell = cell - half_kernel_width + dj*plan->size_x;
        for (di = -half_kernel_width;
             di <= half_kernel_width; 
             di++, p++, ncell++)
        {
          if(PLAN_NOTVALID_BOUNDS(plan,i+di,j+dj))                     
            continue;
