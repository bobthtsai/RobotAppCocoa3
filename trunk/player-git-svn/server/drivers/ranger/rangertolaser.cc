...
int RangerToLaser::Setup (void)
{
	...
	scanData.scan.ranges_count = 360;
	scanData.scan.intensity_count = 360;

	scanData.scan.ranges = new float[scanData.scan.ranges_count];
	scanData.scan.intensity = new uint8_t[scanData.scan.intensity_count];
...
int RangerToLaser::ConvertData (player_msghdr *hdr, void *data)
{
	...
	if (rangeData != NULL)
	{
		if (scanData.scan.ranges_count != rangeCount)
		{
			delete[] scanData.scan.ranges;
			scanData.scan.ranges = NULL;
		}
	...
	{
		if (scanData.scan.intensity_count != intensityCount)
		{
			delete[] scanData.scan.intensity;
			scanData.scan.intensity = NULL;
		}
