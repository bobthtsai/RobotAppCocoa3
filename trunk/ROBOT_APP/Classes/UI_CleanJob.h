...
enum _CLEANJOBMAINITEM_
{
	...
	CLEJOBMAIN_LABWALL,
	CLEJOBMAIN_SIZE,
	CLEJOBMAIN_BTNSPACENUM=2, //每個背景上面元件數量
};
...
private:  
	...
	int iChangRangFlag;
	CCSize winSize;  
	CCArray* m_BtnArray[CLEJOBMAIN_SIZE];//存放所有按鈕
	void InitUI();
...
public:
	...
	void TargetMove_ByMouse(CCSprite *pSprite,CCPoint MousePosition ); //圖釘移動
...
//地圖顯示編輯部分
	...
	void LoadGUIMAPData(CCSprite *pSpriteMap);//載入地圖
	float MAPReSetSize(CCSprite* pMag,float fSize); //設定地圖縮放
	...
	int AddMAPRang_PEN(CCPoint StartSite);	// 增加畫筆
	int  SelMapRang(CCPoint); //判斷點選的位置是否是在範圍元件上
	void SelMapRang_DrawBlackBox(CCDrawNode *pRang);  //去除點選紅框 改為黑框
	...
	void  DrawMapRangByPEN(CCPoint StartSite);// 畫線
	void  DrawMapRang_RealRangByType(CCDrawNode *pRang,pMapRangInfo pRangInfo);//繪製實際範圍(粉紅色區域)
	...
	void  ResetContentSizeRang(CCDrawNode *pRang);//重新設定點選範圍
	...
	int  SaveNewRangInfo(int iWidth,int iHeight,int iAngle,CCPoint stPoint,int iType); // 建立一組新數據
